...
Laporan praktikum ini mengkaji implementasi Form Submission dan Data Validation menggunakan framework Laravel. Tujuan dari praktikum ini adalah untuk mempelajari alur request–response, tahapan validasi terhadap data masukan pengguna, serta mekanisme pengamanan seperti CSRF Protection. Melalui serangkaian eksperimen, peserta mengkaji cara Laravel menangani pengiriman formulir secara aman, menampilkan pesan kesalahan yang mudah dipahami, serta menerapkan formulir multi-langkah dengan menyimpan data sementara di session. Hasil pengujian membuktikan bahwa sistem validasi Laravel dapat meningkatkan aspek keamanan, keandalan, serta kenyamanan pengguna dalam mengisi formulir web.
...
Untuk mengatasi batasan form HTML yang hanya dapat menggunakan GET atau POST, Laravel menerapkan teknik method spoofing. Dengan menambahkan @method('PUT') di dalam form, sebuah input tersembunyi akan dibuat. Input inilah yang nantinya diterjemahkan oleh Laravel untuk mengarahkan request ke route yang sesuai, seolah-olah metode PUT yang digunakan.
...
1. Buat Projek baru laravel pada terminal vscode
...
3. Buat controller FormController.
   ...
   Buat file FormController dengan perintah  commannd artisan: 
...
5.  Setelah itu kita buat view result.blade.php.
...
5. Setelah itu jalankan aplikasi dan tunjukkan hasil.

   Untuk menjalankan aplikasi kita bisa menggunakan perintah artisan berikut:

        php artisan serve

   lalu ctrl+klik `http://127.0.0.1:8000` sehingga akan otomatis ke web browser.
   
   pada URL masukkan `/form` agar masuk ke halaman web yang telah kita buat.

>Screenshot Hasil:
- Form
    ![form](web-lanjut-2024573010038/laporan/laporan5/gambar/submit.png)
    
    
- Hasil submit
    ![submit](web-lanjut-2024573010038/laporan/laporan5/gambar/submit.png)
    

- Form Error (Kosong)
    ![error](web-lanjut-2024573010038/laporan/laporan5/gambar/error.png)
